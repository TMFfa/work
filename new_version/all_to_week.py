...
li_value = 1  # 已修改表格，不必如此，固定为1   /*如果是第一周第一天，用3， 其他每周第一天用2，其余用1*/
kou_fen = 'J'
zhou_fen = 'C'
WEEK = '第二周'

time1 = datetime.now()
wb1 = op.load_workbook('#高三周分.xlsx')  # 周分表******
ws1 = wb1[WEEK]  # 第几周##############################**************************
wb2 = op.load_workbook('#高三男生个人扣分.xlsx')  # 个人扣分总表*********
ws2 = wb2['Sheet1']
time2 = datetime.now()
print(time2-time1)
...
def get_data(cell_list):
    ...
    for cell in cell_list:
        if cell.value is not None:
            li.append(cell)
    li = li[li_value:]  # ***************************同上，只要除表头外的内容, 如果是第一周第一天，用3， 其他每周第一天用2，其余用1
    data = [[], [], []]  # 班级，宿舍，扣分
    for cell in li:
        row = str(cell.row)
        if str(ws2['D'+row].value)+'班' == 'None班':
            continue
        ...
        for class_cell in class_cell_list:
            ...
            if class_ in data[0]:
                ...
                for i in range(10):
                    ...
                    if dor == dormitory:
                        cell = ws1[zhou_fen+str(class_cell.row + i)]  # 周分要扣的那一列，每次都要改***********##############
                        if cell.value is not None:
...
wb1.save('#高三周分.xlsx')  # 保存路径，可以随便写，确认之后再复制到正表
wb1.close()
wb2.close()
